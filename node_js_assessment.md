...
| - | - | - |
| License | Yes | Picking a license is the first thing that should be done before open sourcing the project. There are many different licenses to choose from. The Open Source Initiative (OSI) has them [listed by category](https://opensource.org/licenses/category) to assist in picking one that fits the model of community. |
| Governance | Yes | After picking a licsense, setting a governance model is the next thing that should be established before open sourcing the project.|
...
| Documentation | Yes | Documentation is important on many fronts. It can describe your governance model, describe how to contribute, set a code of conduct, and provide technical documentation about how to get, install, and use the software. Node.js has chosen to use GitHub for documentation as well as other tooling ouside of SCM. The decision is explained [below](#on-the-heavy-use-on-github-tooling). |
...
| Bug reports /<br>Feature requests | Yes | Bug reports and feature requests from the community and their solutions are often the reason for open sourcing a project. Well established guidelines for both are essential. Node.js uses GitHub for this as well. |
...
| Community outreach | Recommended | While [multiple](#on-multiple-community-outreach-tools) community outreach is not esential, it is recommended that the community have some way to engage outside of bug reports and feature requests. This is often for general questions and a building of a community that one feels they "belong" to, regardless of their contribution level or role. |
